...
//        }
    ...
    void getBonus() {
        ...
        if (mUser.getUserDaily().updateGiftReceive(StringHelper.toDBString(idSend))) {
            addResponse(getCommonVector(Bonus.receiveListItem(mUser, DetailActionType.BONUS_FRIEND_GET.getKey(friendId), Bonus.viewGem(1))));
            status();
        } else addErrSystem();
    }
    ...
    void quickGift() {
        ...
        if (updateQuickGift(mUser, update, giftsUpdate, idReceive, lstSend)) {
            ...
            addBonusToastPlus(Bonus.receiveListItem(mUser, DetailActionType.BONUS_FRIEND_SEND.getKey(-1), Bonus.viewGem(countSend)));
