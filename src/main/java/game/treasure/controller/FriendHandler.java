...
import game.treasure.mapping.UserFriendRelationshipEntity;
import game.treasure.mapping.UserGiftEntity;
...
//        }
    ...
    void sendBonus() {
        ...
        if (gift.update()) {
            ...
            addBonusToastPlus(Bonus.receiveListItem(mUser, DetailActionType.BONUS_FRIEND_SEND.getKey(friendId), Bonus.viewItem(Pbmethod.ItemType.CURRENCY.getNumber(),  Pbmethod.ItemKey.GEM, 1)));
            status();
    ...
    void getBonus() {
        ...
        if (mUser.getUserDaily().updateGiftReceive(StringHelper.toDBString(idSend))) {
            addResponse(getCommonVector(Bonus.receiveListItem(mUser, DetailActionType.BONUS_FRIEND_GET.getKey(friendId), Bonus.viewItem(Pbmethod.ItemType.CURRENCY.getNumber(), Pbmethod.ItemKey.GEM, 1))));
            status();
        } else addErrSystem();
    }
    ...
    void quickGift() {
        ...
        if (updateQuickGift(mUser, update, giftsUpdate, idReceive, lstSend)) {
            ...
            addBonusToastPlus(Bonus.receiveListItem(mUser, DetailActionType.BONUS_FRIEND_SEND.getKey(-1), Bonus.viewItem(Pbmethod.ItemType.CURRENCY.getNumber(), Pbmethod.ItemKey.GEM, countSend)));
