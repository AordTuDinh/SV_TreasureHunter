...
public enum TopType {
    ...
    USER_ARENA(5, 0, "ARENA",
            "SELECT *,cup number FROM dson.user WHERE server=%s order by cup desc limit 50",
            "SELECT count(*) number FROM dson.user WHERE server=%s and cup > (SELECT cup FROM dson.user WHERE id=%s)",
            "SELECT *, cup number FROM dson.user WHERE id=%s"),
